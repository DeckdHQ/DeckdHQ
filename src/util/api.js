...
// Like/unlike listing
export const likeListing = body => {
  return post('/api/like-listing', body);
};

// Get liked listings information
export const getLikedListings = params => {
  const queryString = params ? `?${new URLSearchParams(params)}` : '';
  return request(`/api/get-liked-listings${queryString}`, { method: 'GET' });
};

// Check which listings are liked by current user
export const checkLikedListings = body => {
...
export const getOffers = params => {
  const queryString = params ? `?${new URLSearchParams(params)}` : '';
  return request(`/api/get-offers${queryString}`, { method: 'GET' });
};

// Auction APIs
export const getCurrentAuction = () => request('/api/get-current-auction', { method: 'GET' });
export const placeBid = body => post('/api/place-bid', body);
export const getBids = () => request('/api/get-bids', { method: 'GET' });
export const startAuction = body => post('/api/start-auction', body);
export const endAuction = () => post('/api/end-auction', {});
