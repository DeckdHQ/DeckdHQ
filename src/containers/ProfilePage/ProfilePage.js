...
import { getLikedListings } from '../../ducks/like.duck';
import {
  Heading,
  H2,
  H4,
...
export const MainContent = props => {
  ...
  return (
    <div>
          ...
          <ul className={css.listings}>
            {listings.map(l => (
              <li className={css.listing} key={l.id.uuid}>
                <ListingCardWithLike listing={l} showAuthorInfo={false} />
              </li>
            ))}
          </ul>
        </div>
      ...
      {/* Liked Cards Section - only show for current user */}
      {isCurrentUser && likedListings && likedListings.length > 0 ? (
        <div className={listingsContainerClasses}>
          <H4 as="h2" className={css.listingsTitle}>
            <FormattedMessage id="ProfilePage.likedListingsTitle" values={{ count: likedListings.length }} />
          </H4>
          <ul className={css.listings}>
            {likedListings.map(l => (
              <li className={css.listing} key={l.id.uuid}>
                <ListingCard listing={l} showAuthorInfo={true} />
              </li>
            ))}
          </ul>
        </div>
...
export const ProfilePageComponent = props => {
  ...
  useEffect(() => {
    if (isCurrentUser && profileUserId && mounted && onGetLikedListings) {
      onGetLikedListings(profileUserId);
    }
  }, [isCurrentUser, profileUserId, mounted, onGetLikedListings]);
...
const mapDispatchToProps = dispatch => ({
  ...
  onGetLikedListings: userId => dispatch(getLikedListings(userId)),
});
