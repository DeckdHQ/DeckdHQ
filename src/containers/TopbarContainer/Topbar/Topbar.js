...
const TopbarComponent = props => {
  ...
  // Inject Auction link and optional Auction Admin link
  const isOperatorOrAdmin = isLoggedInAs || (authScopes || []).some(s => s === 'admin' || s === 'operator');
  const auctionLink = { group: 'primary', text: 'Auction', type: 'internal', href: '/auction' };
  const auctionAdminLink = isOperatorOrAdmin
    ? { group: 'secondary', text: 'Auction Admin', type: 'internal', href: '/auction-admin' }
    : null;
  const customLinksWithAuction = auctionAdminLink
    ? [...customLinks, auctionLink, auctionAdminLink]
    : [...customLinks, auctionLink];
  ...
  const mobileMenu = (
    <TopbarMobileMenu
      ...
      customLinks={customLinksWithAuction}
