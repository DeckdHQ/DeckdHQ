...
export const InboxPageComponent = props => {
  ...
  const { tab } = params;
  const validTab = tab === 'orders' || tab === 'sales' || tab === 'offers';
  if (!validTab) {
  ...
  const offersTitle = intl.formatMessage({ id: 'InboxPage.offersTitle' });
  const title = isOrders ? ordersTitle : isOffers ? offersTitle : salesTitle;
  ...
  const offersTabMaybe = [
    {
      text: (
        <span>
          <FormattedMessage id="InboxPage.offersTabTitle" />
        </span>
      ),
      selected: isOffers,
      linkProps: {
  ...
  const tabs = [...ordersTabMaybe, ...salesTabMaybe, ...offersTabMaybe];
...
const mapStateToProps = state => {
  ...
  return {
    ...
    getListing: listingId => getMarketplaceEntities(state, [{ id: listingId, type: 'listing' }])[0],
  };
};
